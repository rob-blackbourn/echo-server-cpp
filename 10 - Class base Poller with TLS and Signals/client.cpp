...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    event_loop.add_handler(std::make_unique<FileEventHandler>(console_input, 1024, 1024), "localhost", 1);

    auto console_output = std::make_shared<File>(STDOUT_FILENO, O_WRONLY);
    console_output->blocking(false);
    event_loop.add_handler(std::make_unique<FileEventHandler>(console_output, 1024, 1024), "localhost", 2);
